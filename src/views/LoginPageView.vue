...
<template>
  <BCol class="m-5 p-4 bg-light" v-if="productStore.loggedIn === false">
    ...
    <BForm @submit="onSubmit()" @reset="resetForm()">
      ...
      <BButton class="mt-2 me-4" type="submit" variant="primary">Login</BButton>
      <BButton class="mt-2" type="reset" variant="danger">Reset</BButton>
    </BForm>
    <p class="text-center mt-4">
      Not a member yet?
      <router-link class="text-info" to="/">sign up here</router-link>
    </p>
  </BCol>
  <BCol
  ...
  >
    <h2 class="text-uppercase text-primary m-4">logging in to your account</h2>
    <BSpinner
